	...
	if (result==true) {
    	...
    	$.ajax({
    	    ...
    	    success: function(data){
    	      //$('#preload').html(\"\");
    	      var divactual = document.getElementById(idvar);
    	      divactual.innerHTML = \"\"+data;
    	    }
    	});	
	}
}
...
<div id=\"preload\"></div>
<?php 
echo form_open_multipart(base_url('control/#{@plural}/add_imagen'));
echo form_hidden('id', $this->uri->segment(4));
echo '<input type=\"file\" name=\"adjunto\" id=\"adjunto\" />

<button type=\"submit\">Agregar Imagen a #{@singular}</button>
';
echo form_close();?>

</div>
<div id=\"imagenes_delete\">
...
if($query_imagenes->result()!=\"\"){
    ...
    foreach ($query_imagenes->result() as $imagen) {
        echo '
        <div class=\"thumbnail_remove\" id=\"'.$imagen->id.'\" style=\"float:left; margin:.5em; text-align:center;\">
        <p><img src=\"'.base_url('images-#{@plural}/'.$imagen->filename).'\" width=\"120\" alt=\"\" /></p>
        <p onclick=\"confirma_eliminar('.$imagen->id.')\">Quitar imagen</p>
        </div>';
